...
class Qevix 
{
	...
	const TAG_GLOBAL_ONLY = 6; // Тег может находится только в "глобальной" области видимости (не быть дочерним к другим)
	const TAG_PARENT_ONLY = 7; // Тег может содержать только другие теги
	const TAG_CHILD_ONLY = 8; // Тег может находится только внутри других тегов
	...
	const TAG_PARAM_AUTO_ADD = 12; // Автодобавление параметров со значениями поумолчанию
	...
	/**
	 * КОНФИГУРАЦИЯ: Указывает теги внутри который не нужна авто-расстановка тегов перевода на новую строку
	 *
	 * @param array|string $tags тег(и)
	 */
	...
	public function cfgSetTagBlockType($tags) {
		$this->_cfgSetTagsFlag($tags, self::TAG_BLOCK_TYPE, true);
	}
	...
	/**
	 * КОНФИГУРАЦИЯ: Включает или выключает режим автозамены символов переводов строк строк на тег <br>
	 *
	...
	function parse($text, &$errors)
	{
		...
		$this->textStr = $text;
		$this->textStr = str_replace("\r", "", $this->textStr);
		
		$this->textBuf = $this->strToArray($this->textStr);
